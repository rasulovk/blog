...
window.addEventListener("load", function () {
  var theme = localStorage.getItem("CyberCode_WebTheme");
  ...
  if (theme == "light") {
    ...
    theme = "light";
    localStorage.setItem("CyberCode_WebTheme", theme);
    webTheme.innerHTML = '<i class="lni lni-night"></i>';
  }
...
// Typewriter animation for cycling texts
const animatedText = document.getElementById('animated-text');
const texts = ['Audit Services', 'Consulting'];
...
function typeWriter() {
  ...
  if (isTyping) {
    if (currentCharIndex < words[currentWordIndex].length) {
      animatedText.textContent += words[currentWordIndex][currentCharIndex];
      currentCharIndex++;
      ...
      if (currentWordIndex < words.length) {
        ...
        isTyping = false;
        setTimeout(typeWriter, 3000); // pause after full text, keep it displayed
      }
    }
  } else if (isErasing) {
    if (animatedText.textContent.length > 0) {
      ...
      currentCharIndex = 0;
      isTyping = true;
      setTimeout(typeWriter, 500); // start typing next text
    }
  } else {
